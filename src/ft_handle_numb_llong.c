...
void    ft_helper6(t_printret *ret, va_list argp)
{
    int     i;
    ...
    {
		ret->outvalue = ft_itoa_basell(va_arg(argp, long long), 16);
    ...
    {
		ret->outvalue = ft_itoa_basell(va_arg(argp, long long), 8);
		if (ret->outvalue[0] != 48 && ret->hashable == 1)
		{
			ft_assigner(ret,"0");
			ret->hashable = 0;
...
void    ft_helper10(t_printret *ret, va_list argp)
{
	long	num;
...
void    ft_handle_numb_llong(t_printret *ret, va_list argp)
{
	long    num;
	...
	{
		ret->outvalue = ft_ibase(va_arg(argp, long long), 10);
